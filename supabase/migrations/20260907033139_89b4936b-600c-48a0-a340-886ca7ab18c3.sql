CREATE OR REPLACE FUNCTION public.import_old_crm_leads(_rows jsonb, _skip_daily boolean)
 RETURNS jsonb
 LANGUAGE plpgsql
...
BEGIN
  ...
  SELECT COALESCE(s.default_activation_balance, 250) INTO default_balance
  FROM public.company_settings s WHERE s.company_id = cid;
  default_balance := COALESCE(default_balance, 250);
  ...
  LOOP
    ...
    IF ftd_amount > 0 AND NOT is_invalid AND retention_id IS NULL THEN
      -- No matching agent for this FTD: import the lead anyway and skip the
      -- activation/revenue side so one unknown name cannot abort the import.
      ftd_amount := 0;
    ...
    IF matched_entry_id IS NULL AND NOT _skip_daily THEN
      INSERT INTO public.daily_lead_entries (
        entry_date, source_id, received, invalid, activated, converted,
        reported, cost, notes, company_id, created_by
      ) VALUES (
      ...
      created_entries := created_entries + 1;
    ELSIF matched_entry_id IS NOT NULL AND NOT _skip_daily THEN
    ...
    INSERT INTO public.daily_lead_activations (
      ...
      lead_name, CASE WHEN ftd_amount = default_balance THEN ftd_amount ELSE 0 END, true, cid, ftd_date, false,
    ...
    IF ftd_amount <> default_balance THEN
  ...
  RETURN jsonb_build_object(
    ...
    'updated', filled_count, 'ftds_unassigned', unassigned_count
  );
END;
$function$;

CREATE TABLE IF NOT EXISTS public.daily_entry_import_rows (
...
CREATE UNIQUE INDEX IF NOT EXISTS daily_entry_import_rows_unique
  ON public.daily_entry_import_rows (company_id, row_key);
GRANT SELECT, INSERT, DELETE ON public.daily_entry_import_rows TO authenticated;
GRANT ALL ON public.daily_entry_import_rows TO service_role;
ALTER TABLE public.daily_entry_import_rows ENABLE ROW LEVEL SECURITY;
CREATE POLICY "Company members can view imported daily rows"
  ON public.daily_entry_import_rows FOR SELECT TO authenticated
  USING (company_id = current_company_id());
CREATE POLICY "Company members can record imported daily rows"
  ON public.daily_entry_import_rows FOR INSERT TO authenticated
  WITH CHECK (company_id = current_company_id());
CREATE POLICY "Company members can clear imported daily rows"
  ON public.daily_entry_import_rows FOR DELETE TO authenticated
  USING (company_id = current_company_id());