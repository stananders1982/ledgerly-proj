...
const NONE = "__none__";
const STATUSES = ["new", "no_answer", "voice_mail", "call_back", "wrong_number", "not_interested", "interested", "contacted", "qualified", "lost", "activated"] as const;
const STATUS_LABEL: Record<string,string> = { new:"New", no_answer:"No Answer", voice_mail:"Voice Mail", call_back:"Call Back", wrong_number:"Wrong Number", not_interested:"Not Interested", interested:"Interested", contacted:"Contacted", qualified:"Qualified", lost:"Lost", activated:"FTD" };
...
export function IndividualLeads({ createSignal = 0 }: { createSignal?: number }) {
  ...
  return <div className="space-y-3">
    <div className="flex flex-wrap items-center gap-2">
      ...
      <Select value={status} onValueChange={setStatus}><SelectTrigger className="w-40"><SelectValue/></SelectTrigger><SelectContent><SelectItem value="open">Open leads</SelectItem><SelectItem value="all">All statuses</SelectItem>{STATUSES.map(s=><SelectItem key={s} value={s}>{STATUS_LABEL[s]??s}</SelectItem>)}</SelectContent></Select>
    ...
    {rows.length===0?<EmptyState icon={Plus} title="No leads found" description="Add a lead by details or let affiliates send leads through the intake API." action={<Button onClick={()=>setForm(blank())}>Add lead</Button>}/>:<TableFrame resizeKey="individual-leads" className="w-full"><table className="w-full min-w-[1680px] text-[11px]"><thead className="table-head bg-muted/40 text-muted-foreground"><tr><th className="w-10 p-2"><Checkbox checked={rows.length>0&&rows.every(r=>selected.has(r.id))} onCheckedChange={c=>setSelected(c?new Set(rows.map(r=>r.id)):new Set())}/></th><th className="min-w-44 p-2 text-left">Full Name</th><th className="p-2 text-left">Contact</th><th className="min-w-44 p-2 text-left">E-mail</th><th className="p-2 text-left">Phone</th><th className="p-2 text-left">Country</th><th className="p-2 text-left">Source</th><th className="p-2 text-left">Funnel Name</th><th className="p-2 text-left">Created Date</th><th className="p-2 text-left">Assigned to</th><th className="p-2 text-left">Status</th><th className="p-2 text-left">Balance</th><th className="p-2 text-left">Last comment text</th><th className="p-2 text-right">Actions</th></tr></thead><tbody>{rows.map(l=><tr key={l.id} className="border-t odd:bg-muted/10 hover:bg-accent/30"><td className="p-2"><Checkbox checked={selected.has(l.id)} onCheckedChange={()=>setSelected(s=>{const n=new Set(s);n.has(l.id)?n.delete(l.id):n.add(l.id);return n;})}/></td><td className="p-2 font-medium">{l.name}</td><td className="p-2"><ContactActions phone={l.phone} email={l.email} name={l.name} size="icon"/></td><td className="max-w-52 truncate p-2" title={l.email??""}>{l.email||"—"}</td><td className="p-2 whitespace-nowrap">{l.phone||"—"}</td><td className="p-2 whitespace-nowrap">{geoOf(l)||"—"}</td><td className="p-2">{l.affiliates?.name??l.lead_sources?.name??"—"}</td><td className="max-w-40 truncate p-2" title={funnelOf(l)}>{funnelOf(l)||"—"}</td><td className="p-2 whitespace-nowrap">{fmtDate(l.created_at)}<div className="text-muted-foreground">{new Date(l.created_at).toLocaleTimeString(undefined,{hour:"2-digit",minute:"2-digit"})}</div></td><td className="p-2">{roleKey==="agent"?nameOf(l.employee_id):<Select value={l.employee_id??NONE} onValueChange={v=>patch.mutate({id:l.id,p:{employee_id:v===NONE?null:v}})}><SelectTrigger className="h-8 w-40 border-transparent"><SelectValue/></SelectTrigger><SelectContent><SelectItem value={NONE}>Unassigned</SelectItem>{conversionAgents.map(a=><SelectItem key={a.id} value={a.id}>{a.name}</SelectItem>)}</SelectContent></Select>}</td><td className="p-2"><Select value={l.status} onValueChange={v=>patch.mutate({id:l.id,p:{status:v as LeadStatus}})} disabled={l.activated}><SelectTrigger className="h-8 w-36 border-transparent"><SelectValue/></SelectTrigger><SelectContent>{STATUSES.map(s=><SelectItem key={s} value={s}>{STATUS_LABEL[s]??s}</SelectItem>)}</SelectContent></Select></td><td className="p-2 whitespace-nowrap tabular-nums">{l.activation_id?fmtMoney(balanceOf(l)??0):"—"}</td><td className="max-w-56 truncate p-2" title={l.notes??""}>{lastCommentOf(l)||l.notes||"—"}</td><td className="p-2"><div className="flex justify-end gap-1">{l.activation_id?<Button size="sm" variant="ghost" onClick={()=>navigate({to:"/clients/$id",params:{id:l.activation_id as string}})}><ExternalLink className="h-3 w-3"/> Client</Button>:<Button size="sm" onClick={()=>{setConvert(l);setRetentionId("")}}>Convert</Button>}<Button size="sm" variant="ghost" onClick={()=>setForm({id:l.id,name:l.name,phone:l.phone??"",email:l.email??"",source_id:l.source_id??"",affiliate_id:l.affiliate_id??"",employee_id:l.employee_id??"",status:l.status,notes:l.notes??""})}>Edit</Button><ConfirmDelete onConfirm={()=>remove.mutate(l.id)} label={`Delete ${l.name}?`} description="This removes the lead record. Converted client records are kept."/></div></td></tr>)}</tbody></table></TableFrame>}
