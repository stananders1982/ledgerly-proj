...
export function IndividualLeads({ createSignal = 0 }: { createSignal?: number }) {
  ...
  const patch=useMutation({mutationFn:async({id,p}:{id:string;p:Record<string,unknown>})=>{const {error}=await supabase.from("leads").update(p).eq("id",id);if(error)throw error;},onSuccess:()=>qc.invalidateQueries({queryKey:["individual-leads"]}),onError:(e:any)=>toast.error(e.message)});
